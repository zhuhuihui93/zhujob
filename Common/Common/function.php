<?php
function randomVerificationCode($length = 4, $numeric = 0) {    
  ...
  function uploadOne($imgName,$dirName)
  {
	  if(isset($_FILES[$imgName]) && $_FILES[$imgName]['error'] == 0)
	  {
		  $config = C('IMAGE_CONFIG');
		  $upload = new \Think\Upload($config);
		  $image_ph = new \Think\Image();
		  ...
		  $ret['ok'] = 1;
		  $ret['images'][0] = $logoName = $config['rootPath'].$info[$imgName]['savepath'].$info[$imgName]['savename'];                   
		  $image_ph->open($logoName);
		  return $ret;
	  }
	  else
	  {
