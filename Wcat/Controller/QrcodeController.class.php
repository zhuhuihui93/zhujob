...
class QrcodeController extends Controller {
    ...
    public function createewm(){
        if(IS_POST){
            ...
            $start_secne_id = intval($now_secne_id['scene_id'])+1;
            $end_secne_id = intval($now_secne_id['scene_id'])+intval($create_num);
            $arr = 0;
            for($secne_id=$start_secne_id;$secne_id<=$end_secne_id;$secne_id++){
                ...
                if($json_info['errcode']!=40013){ 
                    ...
                    $data[$arr]['expire_seconds']= !empty($json_info['expire_seconds'])?$json_info['expire_seconds']:0;
                    $data[$arr]['action_name']=$action_name;
                    $data[$arr]['remark']='';
                    $data[$arr]['create_time']=time();
                    $arr++;
                }else{
                    $this->appReturn('fail',103,'aaa');
                }
            }
            if(count($data)>0){
            	foreach ($data as $key => $value) {
	                ...
	                if($res){
	                    ...
	                    $this->appReturn('fail',103,'vvv');
	                }
            	}               
            }else{
            	$this->appReturn('fail',103,'ccc');
            } 
        } 
    }  
