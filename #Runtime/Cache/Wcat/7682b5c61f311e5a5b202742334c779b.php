	...
	<div class="mui-content" style="position: relative;">
		...
		<div id="popover" class="Scavenging-charging-Popup mui-popover">
			...
			<div class="mui-input-row mui-radio mui-left">
				<label>微信支付</label>
				<input id="3" name="radio1" type="radio"  class="rds"/>
			</div>
			<p>使用以下方式支付要先充值哦！</p>
			<div class="mui-input-row mui-radio mui-left">
				<label>积分抵扣 （优惠）</label>
				<input id="1" name="radio1" type="radio" checked="checked" class="rds"/>
			</div>
							...
							mui.ajax('http://www.bjyilongxiang.com/Wcat/Jssdk/users_account',{
								...
								success:function(data){
									...
									if(data.code == 103)
									{
										...
										{
											var oData=eval(data.data)
											oAlertCont.innerHTML='积分还剩 ：'+oData.integral+',需支付'+money+'积分';
										}
      			...
      			mui.ajax('http://www.bjyilongxiang.com/Wcat/Jssdk/order_payment',{
					...
					success:function(data){
						...
						//购买月卡
							...
							$(location).attr('href', '/Wcat/Charge/now_charge');
						}
					},
					error:function(xhr,type,errorThrown){
						//异常处理；
