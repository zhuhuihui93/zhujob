<?php if (!defined('THINK_PATH')) exit();?><form class="dialog-form-page">
	<table cellspacing="0" cellpadding="0" border="0" width="100%">
		<tr>
			<td width="80">上级栏目：</td>
			<td><input class="easyui-combotree" data-options="url:'<?php echo U('Category/public_categorySelect');?>'" type="text" name="info[parentid]" value="<?php echo ($parentid); ?>" style="width:230px;height:24px" /></td>
		</tr>
		<tr>
			<td>栏目名称：</td>
			...
			<td>
				<select name="info[type]" class="easyui-combobox" data-options="editable:false,panelHeight:'auto'" style="width:230px;height:24px">
					<?php if(is_array($typeList)): foreach($typeList as $key=>$type): if(is_array($type['son'])): ?><optgroup label="<?php echo ($type["name"]); ?>">
								<?php if(is_array($type["son"])): foreach($type["son"] as $key2=>$type2): ?><option value="<?php echo ($key2); ?>"><?php echo ($type2["name"]); ?></option><?php endforeach; endif; ?>
							</optgroup>
						<?php else: ?>
							<option value="<?php echo ($key); ?>"><?php echo ($type["name"]); ?></option><?php endif; endforeach; endif; ?>
				</select>
			</td>
		</tr>
		<tr>
		...
		<tr>
			<td>图标：</td>
			<td><input type="text" name="info[icon]" style="width:220px"/></td>
		</tr>
		<tr>
			<td>是否启用：</td>
			<td>
